...
func DiffCinderConfig(ocpConfig string, serviceConfig string) error {
	...
	// Get service Config
	cinderConfig, err := LoadServiceConfig(serviceConfig)
	if err != nil {
		panic(err)
	}
...
func DiffCinderConfigFromPod(ocpConfig string, serviceConfig string, podname string) error {
	// Get ocpConfig
	cinderPatch := LoadCinderOpenshiftConfig(ocpConfig)
	// Get service Config
	cinderConfig, err := GetConfigFromPod(serviceConfig, podname)
	if err != nil {
		panic(err)
	}
