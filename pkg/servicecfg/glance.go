...
func DiffGlanceConfig(ocpConfig string, serviceConfig string, sidebyside bool) error {
	// Get ocpConfig
	glancePatch := LoadGlanceOpenshiftConfig(ocpConfig)
	// Get service Config
	glanceConfig, err := LoadServiceConfig(serviceConfig)
	if err != nil {
		panic(err)
	}

	_, err = CompareIniConfig([]byte(glancePatch), glanceConfig, ocpConfig, serviceConfig)
	if err != nil {
		panic(err)
	}

	if sidebyside {
		_, err = CompareIniConfig(glanceConfig, []byte(glancePatch), serviceConfig, ocpConfig)
		if err != nil {
			panic(err)
		}
	}
...
func DiffGlanceConfigFromPod(ocpConfig string, serviceConfig string, podname string) error {
	// Get ocpConfig
	glancePatch := LoadGlanceOpenshiftConfig(ocpConfig)
	// Get service Config
	glanceConfig, err := GetConfigFromPod(serviceConfig, podname)
	if err != nil {
		panic(err)
	}
